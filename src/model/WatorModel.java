...
public class WatorModel {
	...
	/*
	 * Check to see if the point is in bounds (won't cause out-of-bounds or null
	 * errors)
	 */
			...
			for (int j = 0; j < myWatorModel[i].length; j++) {
				...
				if (myWatorModel[i][j].getTag() == 2) {
					...
					if (!swimAreas.isEmpty()) {
						...
						myWatorModel[previousPoint.x][previousPoint.y] = new Water(WATER_TAG, previousPoint.x,
								previousPoint.y, myWatorModel);
					}
				...
				if (myWatorModel[i][j].getTag() == 1) {
					...
					if (current.isBreeding()) {
						...
						if (!breedAreas.isEmpty()) {
							...
							myWatorModel[breedPoint.x][breedPoint.y] = new Shark(SHARK_TAG, breedPoint.x, breedPoint.y,
									myWatorModel);
					...
					if (!eatAreas.isEmpty()) {
						...
						myWatorModel[eatPoint.x][eatPoint.y] = new Water(WATER_TAG, eatPoint.x, eatPoint.y,
								myWatorModel);
					}
